...
[FIX]
  1. send_sell_alert — sell_label 계산 오류 수정
     (avg_buy_price/avg_buy_price - 1 = 항상 0% → profit_per 파라미터로 수정)
  2. send_max_buy_alert 신규 추가
     → 40회 도달 알림을 코인별 채팅방으로 전송 (기존: MONITOR로 전송되던 버그)
  3. 지정가 매도 체결 완료 알림(send_limit_sell_filled_alert) 신규 추가
     → 기존: 지정가 매도 체결 시 send_sell_alert 미호출로 알림 누락
"""
...
def send_sell_alert(
    ...
    profit_per:    float = 0.0,      # LIMIT 시 지정가 배율 (예: 1.06)
) -> bool:
    """
    매도 체결 즉시 호출. 코인별 채팅방으로 전송.

    [FIX] sell_label 계산 오류 수정.
      기존: avg_buy_price / avg_buy_price - 1 = 항상 0%
      수정: profit_per 파라미터를 받아 퍼센트 표시
    """
    now          = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    coin_symbol  = ticker.split("-")[1]
    ...
    if sell_type == "LIMIT" and profit_per > 0:
        pct          = round((profit_per - 1) * 100)
        header_suffix = f"`{_E_WARN} 40회차 지정가 청산 (+{pct}%)`"
    else:
        header_suffix = f"`시장가 · {buy_count}회차 후 청산`"
...
def send_max_buy_alert(
    ...
    """
    40회(MAX_BUY) 도달 시 지정가 매도 주문 직후 호출.

    [FIX] 기존 _trigger_max_buy_limit_sell에서 send_telegram_msg()만 호출
          → chat_id 미지정으로 MONITOR 채팅방으로 전송되던 버그 수정.
          → 코인별 채팅방으로 전송.
    """
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    pct = round((profit_per - 1) * 100)
    msg = (
        f"{_E_WARN} *[{ticker}] 분할매수 40회 도달 — 지정가 매도 전환*\n"
        f"{_E_CLOCK} `{now}`\n"
        f"──────────────────\n"
        f"📊 평균매수가: `{avg:,} KRW`\n"
        f"🎯 지정매도가: `{limit:,} KRW` (+{pct}%)\n"
        f"📋 매도 체결 대기 중..."
    )
    return send_telegram_msg(msg, chat_id=_chat_id_for(ticker))


def send_limit_sell_filled_alert(
    ticker:        str,
    price:         int,
    amount:        float,
    total:         int,
    ...
    [FIX] 기존: 지정가 매도 체결 완료(chk_sell_order 대기 루프 탈출) 시
          send_sell_alert 호출 없이 카운터만 초기화 → 알림 누락.
          → 전용 함수 추가, 코인별 채팅방으로 전송.
    """
    return send_sell_alert(
        ticker=ticker,
        price=price,
        amount=amount,
        total=total,
...
def send_monitor_report(coin_stats: list) -> bool:
    ...
    coin_stats 딕셔너리 키:
        ticker, cur_price, avg_buy_price, buy_price, sell_price,
        cur_coin, cur_cash, trend, wallet, profit, profit_rate,
        split_count, buy_count, sell_count
    """
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    lines = [
    ...
    for s in coin_stats:
        ...
        if avg and avg > 0:
            ...
            avg_str = f"{avg:,} KRW ({price_vs_avg_sign}{price_vs_avg:.2f}%)"
        else:
            avg_str = "미조회"
        ...
        lines += [
            ...
            f"{'📈' if profit >= 0 else '📉'} 수익: "
            f"`{profit_sign}{profit:,} KRW ({rate_sign}{profit_rate:.2f}%)`",
        ]
...
def send_error_alert(ticker: str, context: str, error: Exception) -> bool:
    """에러 발생 시 호출. 코인별 채팅방으로 전송. ticker 없으면 MONITOR로 폴백."""
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    msg = (
...
if __name__ == "__main__":
    ...
    for ticker, cid in _CHAT.items():
        print(f"{ticker}: {cid if cid else '❌ 미설정'}")
    print(f"MONITOR : {_CHAT_MONITOR if _CHAT_MONITOR else '❌ 미설정'}")
    print()

    TEST_CASES = [
        {"ticker": "KRW-BTC", "price": 90_000_000, "amount": 0.00006667,
         "total": 6_000, "avg_buy_price": 89_500_000,
         "wallet_before": 3_000_000, "wallet_after": 2_994_000, "buy_count": 7},
        {"ticker": "KRW-ETH", "price": 3_850_000, "amount": 0.00259067,
         "total": 9_977, "avg_buy_price": 3_900_000,
         "wallet_before": 1_500_000, "wallet_after": 1_490_023, "buy_count": 3},
        {"ticker": "KRW-XRP", "price": 720, "amount": 8.333,
         "total": 6_000, "avg_buy_price": 715,
         "wallet_before": 500_000, "wallet_after": 494_000, "buy_count": 1},
    ]

    print("=== 매수 알림 테스트 ===")
    for t in TEST_CASES:
        ok = send_buy_alert(**t)
        print(f"{t['ticker']} 매수: {'✅' if ok else '❌'}")

    print()
    print("=== 매도 알림 테스트 ===")
    SELL_CASES = [
        {"ticker": "KRW-BTC", "price": 91_000_000, "amount": 0.00006667,
         "total": 6_066, "avg_buy_price": 89_500_000,
         "profit": 66, "profit_rate": 1.10,
         "wallet_before": 2_994_000, "wallet_after": 3_000_066,
         "buy_count": 7, "sell_type": "MARKET", "profit_per": 0.0},
        {"ticker": "KRW-ETH", "price": 4_017_000, "amount": 0.00259067,
         "total": 10_404, "avg_buy_price": 3_900_000,
         "profit": 427, "profit_rate": 4.27,
         "wallet_before": 1_490_023, "wallet_after": 1_500_427,
         "buy_count": 3, "sell_type": "MARKET", "profit_per": 0.0},
        {"ticker": "KRW-XRP", "price": 757, "amount": 8.333,
         "total": 6_308, "avg_buy_price": 715,
         "profit": 308, "profit_rate": 5.17,
         "wallet_before": 494_000, "wallet_after": 500_308,
         "buy_count": 40, "sell_type": "LIMIT", "profit_per": 1.06},
    ]
    for t in SELL_CASES:
        ok = send_sell_alert(**t)
        print(f"{t['ticker']} 매도({t['sell_type']}): {'✅' if ok else '❌'}")

    print()
    print("=== 40회 도달 알림 테스트 ===")
    ok = send_max_buy_alert("KRW-BTC", avg=89_500_000, limit=94_870_000, profit_per=1.06)
    print(f"KRW-BTC 40회 도달: {'✅' if ok else '❌'}")

    print()
    print("=== MONITOR 리포트 테스트 ===")
    ok = send_monitor_report([
        {"ticker": "KRW-BTC", "cur_price": 90_000_000, "avg_buy_price": 89_000_000,
         "buy_price": 89_730_000, "sell_price": 92_700_000,
         "cur_coin": 0.00123456, "cur_cash": 50_000, "trend": "run-up",
         "wallet": 160_800, "profit": 10_800, "profit_rate": 7.20,
         "split_count": 7, "buy_count": 42, "sell_count": 5},
        {"ticker": "KRW-ETH", "cur_price": 3_850_000, "avg_buy_price": 3_900_000,
         "buy_price": 3_838_450, "sell_price": 0,
         "cur_coin": 0.00259067, "cur_cash": 290_000, "trend": "down",
         "wallet": 299_973, "profit": -27, "profit_rate": -0.01,
         "split_count": 12, "buy_count": 18, "sell_count": 3},
        {"ticker": "KRW-XRP", "cur_price": 720, "avg_buy_price": 710,
         "buy_price": 717, "sell_price": 741,
         "cur_coin": 13.88, "cur_cash": 1_000, "trend": "up",
         "wallet": 100_994, "profit": 994, "profit_rate": 0.99,
         "split_count": 3, "buy_count": 95, "sell_count": 22},
    ])
    print(f"MONITOR 리포트: {'✅' if ok else '❌'}")